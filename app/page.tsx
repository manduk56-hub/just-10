...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="records" aria-label="이번 방문의 기록"><div className="best-record"><span className={`record-icon ${best !== null ? 'obtained' : ''}`} aria-hidden="true"><img src="/textures/diamond.png" alt="" /></span><div><span className="tiny-caption">BEST PRECISION</span><strong>{best === null ? <span className="no-record">첫 기록에 도전해 보세요</span> : <>±{format(best)}<small>초</small></>}</strong></div></div><div className="recent-records"><div className="recent-title">최근 도전 <span>{attempts.length}</span></div><div className="attempt-list">{attempts.length === 0 ? <span className="empty-record">아직 기록이 없어요. 첫 블록을 캐볼까요?</span> : attempts.slice(0, 5).map((a, i) => <span key={`${attempts.length}-${i}`} className={`attempt ${Math.abs(a.time - TARGET) <= 100 ? 'good' : ''}`}>{format(a.time)}<small>s</small>{i === 0 && <span className="new-dot" />}</span>)}</div></div><span className="session-note">이번 방문의 기록</span></section>
    </main>
