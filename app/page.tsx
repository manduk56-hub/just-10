...
function AchievementDialog({ time, onClose, onNext }: { time: number; onClose: () => void; onNext: () => void }) {
  const dialog = useRef<HTMLDialogElement>(null);
  const dismiss = () => { dialog.current?.close(); onClose(); };
  const next = () => { dialog.current?.close(); onNext(); };
  useEffect(() => {
  ...
  return <dialog ref={dialog} className="achievement-dialog" aria-labelledby="achievement-title" aria-describedby="achievement-description" onCancel={event => { event.preventDefault(); dismiss(); }} onKeyDown={event => event.stopPropagation()}>
    ...
    <Button variant="ghost" className="achievement-retry" onClick={next}><img src="/textures/diamond_pickaxe.png" alt="" />다음 참가자 시작</Button>
  </dialog>;
}
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    {achievementOpen && success && <AchievementDialog time={elapsed} onClose={() => setAchievementOpen(false)} onNext={act} />}
  </div>;
}
